...
function netflixChapterCard(c){
  ...
  return `<a class="nf-card ${statusClass}" href="#/capitulos/${c.id}" aria-label="Capitulo ${c.id} - ${esc(chapterTitle(c))}" style="--card-img:url('${assetUrl(['chapters',`cap-${String(c.id).padStart(2,'0')}.jpg`])}')"><span class="nf-card-bar"><i style="width:${x}%"></i></span><small>${status}</small></a>`;
}
function list(){
