...
function netflixChapterCard(c){
  ...
  return `<a class="nf-card ${statusClass}" href="#/capitulos/${c.id}" style="--card-img:url('${assetUrl(['chapters',`cap-${String(c.id).padStart(2,'0')}.jpg`])}')"><span class="nf-number">${String(c.id).padStart(2,'0')}</span><span class="nf-play">▶</span><span class="nf-title">${esc(chapterTitle(c))}</span><span class="nf-card-bar"><i style="width:${x}%"></i></span><small>${status}</small></a>`;
}
function list(){
